...
//      val res = new ListBuffer[(String, List[String])]()
      ...
      val SPLITER = ","
      val tagList = new ListBuffer[(String, Long)]()
      for (_cell <- result.rawCells) {
        ...
        tagList.append((qualifier, timestamp))
      ...
      //
      val b = tagList.sortBy(_._2)
      val tagTemp = new ListBuffer[String]
      for (tag <-  tagList) {
        for (t <- tagTemp) {
          if (tag._1.substring(0,5).equals(t.substring(0,5))) {
